...
export default function Login() {
    ...
    const { user } = useAuth(); // Assuming useAuth is already imported, if not add the import

    useEffect(() => {
        // Only redirect if we have a valid user AND strictly ensuring we are not loading.
        // The persistence of the login button on dashboard suggests the auth context might be slow to update or race condition.
        if (user) {
            // Force a small delay or rely on state.
            // Actually, simply navigating when user is present SHOULD work.
            // If the user sees "Login" on dashboard, it means dashboard's navbar thinks user is null.
            // This is the AuthContext propagation time.
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-orange-50 p-4">
            <div className="w-full max-w-md rounded-xl bg-white p-8 shadow-lg">
                ...
                <h3 className="mb-4 text-center text-xl font-semibold text-gray-700">Welcome Back</h3>
